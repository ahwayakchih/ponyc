...
#ifdef _MSC_VER
#  pragma warning(push)
// 4003/4244/4267/4624/4141/4146 are the standard noisy-diagnostic suppressions
// for pulling LLVM/clang headers in under MSVC, carried as a set from the
// existing LLVM-include pattern.
...
// 4805 is the one addition beyond that inherited set: clang's own headers
// (TemplateName.h, TypeBase.h) mix bool into | expressions.
  ...
  std::vector<const char*>& args, errors_t* errors)
{
  ...
  // (including <sysroot>/include, which musl-style sysroots use). The
  // -internal-externc-isystem variant the driver uses for these dirs only
  // affects implicit extern "C" wrapping in C++ compiles; shims are compiled
  // -x c, where it is inert — kept here for byte-parity with the driver.
  bool any = false;
  ...
  const char* obj, const std::vector<const char*>& common_args)
{
  ...
  // Invocation parsing runs outside the crash-recovery context below: gencshim
  ...
  if(!clang::CompilerInvocation::CreateFromArgs(ci.getInvocation(), args,
    ci.getDiagnostics()))
  {
    // The argv is built by gencshim, so a parse failure is a ponyc bug; make
...
// Append the cc1 target/ABI arguments derived from the pass options: the target
// triple, CPU, target features (one -target-feature per comma-separated entry
// in the LLVM target-machine string), ABI, and PIC relocation model. These must
// match what the Pony codegen targets, or the shim object is ABI-incompatible
// with the program it links into. Factored out of gencshim so CShimTest can
// assert the exact argv -- in particular that an empty CPU omits -target-cpu
// entirely (codegen.c defaults a cross build's CPU to ""), and that a
// comma-separated feature string round-trips to one -target-feature each. The
// rest of the cc1 invocation (optimization level, debug info, unwind/frame
// hygiene) is not ABI-determining and stays inline in gencshim.
void cshim_target_args(pass_opt_t* opt, std::vector<const char*>& args)
{
  args.push_back("-triple");
  args.push_back(opt->triple);

  if(opt->cpu != NULL && opt->cpu[0] != '\0')
  {
    args.push_back("-target-cpu");
    args.push_back(opt->cpu);
  }
...
bool gencshim(ast_t* program, pass_opt_t* opt)
{
  ...
  // gencshim is not an AST pass with per-node re-entry flags, so guard
  ...
  // objects a second time. The flag is set up front, so a re-run after a
  // FAILED run (the orphan-directive error below, or a partial compile) returns
  // true and proceeds as if the shims succeeded — the orphan error won't
  // re-fire and an incomplete object set could be linked. That is safe only
  // because no in-tree caller re-runs after gencshim returns false; a future
  // resumable-compile embedder must not rely on retrying a failed run.
  if(program_gencshim_done(program))
    return true;

  program_set_gencshim_done(program);

  // Single package walk: detect whether any shims exist (the fast-path
  // signal) and, in the same pass, reject cdefine:/cincludedir: directives in
  ...
  // per directive. Consequence: unlike the other cdefine:/cincludedir: errors
  // (bad identifier, duplicate name) which fire in the scope pass, this orphan
  // error surfaces only at PASS_C and later, so `ponyc --pass verify` won't
  // catch it. A real build always runs PASS_C, so it always fires there.
  bool any = false;
    ...
    package = ast_sibling(package))
  {
    ...
    if(has_flags && !has_sources)
    {
      ...
      if(site != NULL)
        ast_error(errors, site, "cdefine:/cincludedir: in a package with no C "
          "source files; these flags only apply to .c files in the same "
          "package, so this directive has no effect");
      else
        errorf(errors, NULL, "cdefine:/cincludedir: in a package with no C "
          "source files");
  ...
  // target backends at process startup via codegen_llvm_init, before gencshim
  ...
  // Target/ABI flags (must match the Pony codegen target). Factored out so it
  // has an asserting test -- see cshim_target_args above.
  cshim_target_args(opt, common);
  ...
  // codegen creates the output directory too, but gencshim runs first; without
